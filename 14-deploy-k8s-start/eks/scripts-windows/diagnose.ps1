# Script de diagnóstico para problemas de despliegue en EKS

Write-Host "=== Diagnóstico del despliegue en EKS ===" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "---"
if ($clientPod) {
    kubectl exec $clientPod -- env | Select-String "EXPENSE"
    if ($LASTEXITCODE -ne 0) {
        Write-Host "No se encontró variable EXPENSE_SVC" -ForegroundColor Red
    }
} else {
    Write-Host "No se encontró pod de expense-client" -ForegroundColor Red
}
